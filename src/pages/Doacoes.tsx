...
export function Doacoes() {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      if (err instanceof RemoteApiError) {
        ...
        setError('Erro desconhecido ao buscar doações.');
      }
    } finally {
      setLoading(false);
    }
  ...
  const doacoesFiltradas = useMemo(() => {
    ...
    return doacoes.filter((d) => {
      const texto = [d.id, d.idDoador, String(d.valor), d.dtDoacao].filter(Boolean).join(' ').toLowerCase();
  ...
  const onSubmit: SubmitHandler<DoacaoPayload> = async (data) => {
    setCreating(true);
    try {
      await doacaoService.criar({ ...data, valor: Number(data.valor) });
      reset();
      setShowForm(false);
      setSuccessMsg(`Doação de ${formatarValor(Number(data.valor))} registrada com sucesso!`);
      setTimeout(() => setSuccessMsg(''), 4000);
      carregar();
    } catch (err: unknown) {
      alert('Erro ao registrar doação: ' + (err instanceof Error ? err.message : String(err)));
    } finally {
      setCreating(false);
    }
  };
  ...
  return (
    <div className="max-w-285 w-full mx-auto my-6 xs:my-8 sm:my-10 lg:my-12 xl:my-14 px-3 xs:px-4 sm:px-6 lg:px-7 xl:px-8">
      <PageHero
        badge="Financeiro"
        title="Doações"
        description="Registre e acompanhe todas as doações recebidas pela clínica."
        layout="split"
        actions={
          <Button
            id="btn-nova-doacao"
      ...
      {showForm && (
        <section className="bg-surface rounded-xl border border-border shadow-sm p-6 xs:p-8 mb-7">
          <SectionHeader title="Registrar Nova Doação" />
          <form
            id="form-doacao"
            aria-label="Formulário de cadastro de doação"
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                min="0"
                placeholder="0,00"
                className={`${inputBase} ${errors.valor ? 'border-red-500' : ''}`}
                {...register('valor', { required: 'Preencha o valor', min: { value: 0.01, message: 'Valor deve ser maior que zero' } })}
              />
              {errors.valor && <p className="text-red-600 text-[0.82rem] mt-1">{errors.valor.message}</p>}
            </div>

            <div>
              <label htmlFor="doac-data" className="block font-semibold text-[0.9rem] text-brand-text mb-1.5">Data da Doação</label>
              <input
                id="doac-data"
                type="date"
            ...
            <div className="xs:col-span-2 flex flex-col xs:flex-row gap-3">
              <Button type="submit" disabled={creating} id="btn-salvar-doacao">
                {creating ? 'Salvando...' : 'Registrar Doação'}
              </Button>
      ...
      <section className="bg-surface rounded-xl border border-border shadow-sm p-6 xs:p-8">
        <div className="flex flex-col xs:flex-row xs:items-center justify-between gap-3 mb-6">
          <SectionHeader title="Lista de Doações" />
          <button
        ...
        {!loading && doacoes.length > 0 && (
          <div className="flex justify-between text-[0.85rem] text-muted mb-4">
            <p>Mostrando {doacoesFiltradas.length} de {doacoes.length} doações</p>
        ...
        {loading ? (
          ...
          <p className="text-muted text-center py-12">Nenhuma doação registrada. Adicione a primeira!</p>
        ) : doacoesFiltradas.length === 0 ? (
          <p className="text-muted text-center py-12">Nenhuma doação encontrada para a busca aplicada.</p>
        ) : (
            ...
            <table className="w-full text-[0.88rem]">
              <thead className="bg-brand-bg">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 font-semibold text-brand-text/70">Ações</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-border">
                {doacoesPagina.map((d) => (
                  <tr key={d.id} className="bg-white hover:bg-brand-bg/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right font-bold text-emerald-700">{formatarValor(d.valor)}</td>
        ...
        {!loading && doacoes.length > 0 && (
          <div className="mt-4 p-4 rounded-lg bg-emerald-50 border border-emerald-200 flex items-center justify-between">
            ...
            <span className="text-emerald-700 font-extrabold text-[1.1rem]">
              {formatarValor(doacoes.reduce((acc, d) => acc + (Number(d.valor) || 0), 0))}
            </span>
          </div>
        )}
      </section>
